...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        best = [action for action in actions if self.getQValue(state, action) == max]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        q = 0
        for i, j in self.featExtractor.getFeatures(state, action).items():
          q += self.weights[i] * j
        return q
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i, j in self.featExtractor.getFeatures(state, action).items():
          self.weights[i] += self.alpha * (diff * j)
        return self.weights
